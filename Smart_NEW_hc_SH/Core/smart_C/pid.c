...
#define MAX_OUTPUT 2000 // 积分输出限幅
#define MIN_OUTPUT -2000
#define MIN_TARGET -1000 // 电机目标值范围
#define MAX_TARGET 1000
#define PWM_MAX_VALUE 2000                                          // PWM 输出最大值
#define PID_MAX_OUTPUT 2000                                          // PID 输出最大绝对值
...
float PID_realize(int index)
{
  ...
  static int pwm_output[4];
  int actual_val[4];
  actual_val[index] = Get_Encoder(index); // 获取当前实际值
  ...
  // 将输出值转换为 PWM 输出
  pwm_output[index] = (float)(pid[index].output_val * SCALE_FACTOR);
